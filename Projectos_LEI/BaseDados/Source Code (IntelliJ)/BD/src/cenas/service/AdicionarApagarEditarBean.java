...
public class AdicionarApagarEditarBean {
    ...
    public boolean editaritem(){
        try {
            if (server.modificar_item((Integer.parseInt(id_reuniao)), (Integer.parseInt(id_item)), descricao_item, (user.getId())))
                return true;
            else
                return false;
    ...
    public boolean apagaritem(){
        try {
            if (server.eliminar_item((Integer.parseInt(id_reuniao)), (Integer.parseInt(id_item)),user.getId()))
                return true;
            else
                return false;
        } catch (FileNotFoundException ex) {
    ...
    public boolean adicionaritem(){
        try {
            if (server.adicinar_item((Integer.parseInt(id_reuniao)), descricao_item,user.getId()) == null)
                return false;
            else
                return true;
        } catch (FileNotFoundException ex) {
    ...
    public boolean addkey(){
        ...
        try {
            if (server.addkey((Integer.parseInt(id_reuniao)), (Integer.parseInt(id_item)), key,user.getId()))
                return true;
            else
                return false;
        } catch (RemoteException ex) {
